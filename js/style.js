...
//关闭增加消费记录 分析综述
    ...
    var user_box1 = $('.user_box1');
    var user_box1a = user_box1.find('h1 a');
    ...
    cht(fdjs,user_box1,'.user_box');
    cht(user_box1a,user_box,'.user_box1');
    ...
    //积分
    var jfShow = $('.jf_show');
    var jfShowLi = jfShow.find('ul li');
    var jjJf = $('.jj_jf');
    var num = 0;
    jfShowLi.on('click',function(){
        if(num === 0){
            jjJf.css({
               opacity : '1',
                bottom : '0.49rem',
    ...
    var jjJfInp = $('.jj_jf').find('input');
    var jfbox1A = $('.jf_box1 h1').find('a');
    var jfBox1 = $('.jf_box1');
    var jfBox1D = jfBox1.children('div');

    jjJfInp.on('click',function(){
        var jjJfInpZ = $(this).index();
        $(this).parents('.jf_box').hide();
        jfBox1.show();
        jfBox1D.eq(jjJfInpZ).show();
    });
    jfbox1A.on('click',function(){
        jfBox1.hide();
        $('.jf_box').show();
